...
// Uses WebCrypto API for ES256 (ECDSA P-256) and EdDSA (Ed25519) verification
...
async function jwkThumbprint(jwk) {
  ...
  if (jwk.kty === 'EC' && jwk.crv === 'P-256' && jwk.x && jwk.y) {
    ...
    json = JSON.stringify({ crv: jwk.crv, kty: jwk.kty, x: jwk.x });
  } else {
...
async function jwkToCryptoKey(jwk, algorithm) {
  if (algorithm === 'ES256') {
    if (!jwk || jwk.kty !== 'EC' || jwk.crv !== 'P-256' || !jwk.x || !jwk.y) {
      throw new Error('Invalid P-256 JWK for ES256');
    }
    ...
    if (!jwk || jwk.kty !== 'OKP' || jwk.crv !== 'Ed25519' || !jwk.x) {
      throw new Error('Invalid Ed25519 JWK for EdDSA');
    }
...
export async function verifyReceipt({ receipt, jwks }) {
  try {
    ...
    // Verify signature using WebCrypto based on algorithm
    let isValid = false;
    ...
    if (header.alg === 'ES256') {
      ...
      const cryptoKey = await jwkToCryptoKey(key, 'ES256');
      ...
      const cryptoKey = await jwkToCryptoKey(key, 'EdDSA');
      isValid = await crypto.subtle.verify(
        { name: 'Ed25519' },
        cryptoKey,
        signature,
